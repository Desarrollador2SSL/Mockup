// ================================
// Cliente - Modificaciones JavaScript - VERSIÓN FINAL CORREGIDA
// ===============================
...
// =============================================
// NUEVA MODIFICACIÓN - Step Functions CORREGIDAS
// =============================================
...
// Setup Step 1
function setupStep1() {
    // Las tarjetas OC ya tienen onclick en el HTML
    // No necesitamos agregar event listeners adicionales
    console.log('Step 1 inicializado');
}

// Select OC - función global para ser llamada desde HTML
window.selectOC = function(ocNumber, element) {
    console.log('Seleccionando OC:', ocNumber);
...
// Select Modification Type - función global para ser llamada desde HTML
window.selectModificationType = function(type, element) {
    console.log('Seleccionando tipo de modificación:', type);
    ...
    // Store selected type - IMPORTANTE: actualizar la variable global
...
window.validateStep2 = function() {
    ...
    const reasonValid = reason.value.trim().length >= 50;
    const isValid = selectedModificationType !== null && reasonValid;
    
    console.log('Validación Step 2:', {
        selectedModificationType,
        reasonLength: reason.value.trim().length,
        isValid
    });
...
// Go to Step - CORREGIDA
window.goToStep = function(stepNumber) {
    console.log('Intentando ir al paso:', stepNumber);
    console.log('Paso actual:', currentStep);
    console.log('Variables globales:', { selectedOC, selectedModificationType });
...
// Validate current step - CORREGIDA
function validateCurrentStep() {
    console.log('Validando paso actual:', currentStep);
    
    switch(currentStep) {
        case 1:
            const valid1 = selectedOC !== null;
            console.log('Paso 1 válido:', valid1, 'OC seleccionada:', selectedOC);
            showNotification('Paso de validacion 1', 'warning');
            return valid1;
        case 2:
            //const reason = document.getElementById('modificationReason');
            //const valid2 = selectedModificationType !== null && reason && reason.value.trim().length >= 50;
            //console.log('Paso 2 válido:', valid2, 'Tipo:', selectedModificationType, 'Razón length:', reason?.value.trim().length);
            showNotification('Paso de validacion 2', 'warning');
            return valid2;
        case 3:
            // Para el paso 3, por ahora permitimos continuar siempre
            // En producción, verificaríamos que al menos un checkbox esté marcado
            showNotification('Paso de validacion 3', 'warning');
            return true;
        case 4:
            showNotification('Paso de validacion 4', 'warning');
            return true;
        default:
            return true;
    }
...
// Setup Step 2
function setupStep2() {
    console.log('Configurando Paso 2');
    ...
    // Reset modification type si no fue seleccionado previamente
    ...
    if (reasonTextarea) {
        ...
        reasonTextarea.removeEventListener('input', validateStep2);
        // Agregar nuevo listener
        reasonTextarea.addEventListener('input', validateStep2);
    }
}

// Setup Step 3 - MEJORADA
function setupStep3() {
    console.log('Configurando Paso 3, tipo de modificación:', selectedModificationType);
    ...
    if (selectedModificationType === 'quantity') {
        ...
        console.warn('No modification type selected, defaulting to quantity');
        quantitySection.style.display = 'block';
        dateSection.style.display = 'none';
    }
    ...
    // Marcar algunos checkboxes por defecto para facilitar las pruebas
    const checkboxes = document.querySelectorAll('#stepContent3 input[type="checkbox"]');
...
// Handler para inputs de cantidad
...
// Update Item Difference - función global
window.updateItemDifference = function(input, originalQty, unitPrice) {
...
// Calculate Total Impact - función global
window.calculateTotalImpact = function() {
    let totalImpact = 0;
...
// Setup Step 4
function setupStep4() {
    console.log('Configurando Paso 4');
    ...
    if (acceptTerms) {
        acceptTerms.removeEventListener('change', validateStep4);
        acceptTerms.addEventListener('change', validateStep4);
    }
}

// Validate Step 4 - función global
...
// Handle File Upload - función global
    ...
    Array.from(files).forEach(file => {
        ...
        if (uploadedFilesDiv) {
            ...
            fileElement.innerHTML = `
                ...
                <button onclick="removeFile('${file.name}')" style="background: none; border: none; color: #ef4444; cursor: pointer;">
...
// Submit Modification - función global
window.submitModification = function() {
    ...
    setTimeout(() => {
        ...
        const newModElement = document.getElementById('newModNumber');
        if (newModElement) {
            newModElement.textContent = newModNumber;
        }
